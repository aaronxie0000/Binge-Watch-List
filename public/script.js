...
async function sessionIDinput(e) {
  ...
  if (existing === true) {
    alertMessage.textContent =
      "This SessionID has been used. If this is you or the EXAMPLE, simply exit and scroll to continue";
...
function updateMoviePoster(movieData) {
  if (movieData == null) {
    alertMessage.textContent =
      "Invalid movie name! Sure you got the rigth movie or tv show name? Try again!";
    alertBox.classList.remove("hidden");
    return;
  }
...
function getCheckedMovies() {
  ...
  try {
    ...
    alertMessage.textContent =
      "Please make sure you have searched and selected at least one movie";
    alertBox.classList.remove("hidden");
  }
...
async function pushToDataBase(movieObj) {
  ...
  if (userID == null) {
    alertMessage.textContent =
      "Please re-enter your SessionID before submitting";
    alertBox.classList.remove("hidden");
    return;
  }
...
async function updateDisplayMovies(userID) {
  ...
  const removeToWatchList = document.querySelectorAll(
    ".toWatchList > li:not(.listTitle)"
  );
  removeToWatchList.forEach((ele) => ele.remove());

  const removeDataList = document.querySelectorAll(
    ".infoList > tr:not(.infoListTitle)"
  );
...
async function dragEnd() {
  ...
  const movieItems = childElements.filter(
    (ele) =>
      ele.className == "listMovieItem" ||
      ele.className == "listMovieItem watchedItem"
  );
...
function getDragToElement(container, mouseY) {
  const eligbleDragTo = Array.from(
    container.querySelectorAll(".listMovieItem:not(.listTitle):not(.dragging)")
  );
